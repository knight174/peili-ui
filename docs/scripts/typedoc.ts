...
import { Application, TSConfigReader, ReflectionKind, ProjectReflection } from 'typedoc';
    ...
    module.children?.forEach(sub => {
      ...
      if (sub.kind === ReflectionKind.Class) {
        moduleConfig.items?.push({
          text: sub.name,
          link: `/api/classes/${module.name}.${sub.name}`,
        });
      } else if (sub.kind === ReflectionKind.Interface) {
        moduleConfig.items?.push({
          text: sub.name,
          link: `/api/${module.name}/interfaces/${sub.name}`,
        });
      }
    });
  });
