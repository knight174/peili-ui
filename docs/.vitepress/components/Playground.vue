...
async function fetchUiVersions() {
  const res = await fetch('https://data.jsdelivr.com/v1/package/npm/peili-ui');
  const { versions } = (await res.json()) as { versions: string[] };
...
function setUiVersion(version: string) {
  ...
  store.setImportMap({
    imports: {
      'peili-ui': `https://fastly.jsdelivr.net/npm/peili-ui@${version}/dist/peili-ui.full.min.mjs`,
    },
  });
  // 加载组件库的全量样式
  previewOptions.headHTML = `<link rel="stylesheet" href="https://fastly.jsdelivr.net/npm/peili-ui@${version}/dist/style/index.css">`;
}
...
<template>
  <div v-if="isMounted">
    <Repl :store="store" :editor="Monaco" :auto-resize="true" :clear-console="false"
      :preview-options="previewOptions" />

    <Teleport to=".VPNavBarSearch">
      <div class="flex items-center text-14px">
        <label class="playground-label">PeiliUI: </label>
...
.playground-select {
  ...
  margin-left: 8px;
  appearance: auto;
  border: 1px solid rgb(var(--pl-color-bd_base));
}
</style>