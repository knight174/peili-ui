...
export interface GenerateVueConfigOptions extends GenerateConfigOptions {
  ...
  /** 传递给组件库 UnoCSS 预设的选项 */
  presetPeiliuiOptions?: PeiliuiPresetOptions;
}
...
export async function generateVueConfig(
  ...
  const { pluginUno = true, presetPeiliuiOptions, presetUnoOptions } = customOptions || {};

  const configPreset: UserConfig = {
    plugins: [
      pluginUno // eslint-disable-next-line operator-linebreak
        ? unocss({
            ...
            presets: [
              ...
              peiliuiPreset(presetPeiliuiOptions),
            ],
            transformers: [
